...
class _UploadScreenState extends State<UploadScreen> {
  ...
  @override
  void dispose() {
    _namaCtrl.dispose(); _ketCtrl.dispose(); super.dispose();
  }
  ...
  Future<void> _pickFiles() async {
    ...
    if (result != null) {
      setState(() => _files.addAll(result.paths.whereType<String>().map((e) => File(e))));
    }
  }

  Future<void> _startUpload() async {
    final nama = _namaCtrl.text.trim();
    if (nama.isEmpty) { _showSnack('Nama wajib diisi'); return; }
    if (_activeDivisi.isEmpty) { _showSnack('Pilih divisi terlebih dahulu'); return; }
    if (_files.isEmpty) { _showSnack('Pilih file terlebih dahulu'); return; }
    ...
    setState(() {
      _uploading = true;
      _statuses = _files.map((f) => _FileStatus(file: f)).toList();
    });
    ...
    if (done == _files.length) {
      _showSuccessDialog(done);
      setState(() { _files = []; _statuses = []; _ketCtrl.clear(); });
    }
  }
  ...
  void _showSuccessDialog(int count) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('Upload Berhasil!', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 6),
        Text('$count file berhasil dikirim', style: const TextStyle(color: Color(0xFF5A8A6A))),
        const SizedBox(height: 4),
        Text(_activeDivisi, style: const TextStyle(color: AppTheme.gold)),
  ...
  void _showSnack(String msg) => ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text(msg), backgroundColor: AppTheme.greenMid,
      ...
      body: SafeArea(child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Center(child: Column(children: [
            const Text('بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ',
              style: TextStyle(color: AppTheme.gold, fontSize: 20, fontFamily: 'serif')),
            const SizedBox(height: 4),
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: 'Nama Lengkap *', prefixIcon: Icon(Icons.person_outline, color: AppTheme.greenRim)),
          ),
          const SizedBox(height: 10),

          // Divisi chips
          const Text('Divisi *', style: TextStyle(color: AppTheme.goldLight, fontSize: 12)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...AppConstants.divisiList.map((d) => DivisiChip(
              label: d,
              selected: !_customDivisi && _divisi == d,
              onTap: () => setState(() { _divisi = d; _customDivisi = false; }),
            )),
            DivisiChip(
              label: '✏️ Lainnya',
              selected: _customDivisi,
              onTap: () => setState(() { _customDivisi = true; _divisi = 'custom'; }),
            ),
          ]),
          if (_customDivisi) ...[
            const SizedBox(height: 8),
            TextFormField(
              ...
              style: const TextStyle(color: Colors.white),
              decoration: const InputDecoration(labelText: 'Nama Divisi Custom', prefixIcon: Icon(Icons.edit, color: AppTheme.greenRim)),
            ),
          ],
          const SizedBox(height: 10),
          TextFormField(
            ...
            style: const TextStyle(color: Colors.white),
            maxLines: 2,
            decoration: const InputDecoration(labelText: 'Keterangan (opsional)', prefixIcon: Icon(Icons.notes, color: AppTheme.greenRim)),
          ),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: AppTheme.greenRim, style: BorderStyle.solid, width: 1.5),
              ),
              child: Column(children: [
                const Text('📁', style: TextStyle(fontSize: 36)),
                const SizedBox(height: 8),
                const Text('Tap untuk pilih file', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500)),
                const SizedBox(height: 4),
                Text('JPG, PNG, HEIC, MP4, MOV, MKV...',
                  style: const TextStyle(color: Color(0xFF5A8A6A), fontSize: 12)),
                const SizedBox(height: 4),
                const Text('Maks. 500 MB/file', style: TextStyle(color: AppTheme.gold, fontSize: 11)),
              ]),
            ),
          ),
          ...
          if (_files.isNotEmpty) ...[
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('${_files.length} file dipilih',
                style: const TextStyle(color: AppTheme.goldLight, fontSize: 13, fontWeight: FontWeight.w500)),
              if (!_uploading)
                TextButton(onPressed: () => setState(() { _files = []; _statuses = []; }),
            ...
            ..._files.asMap().entries.map((e) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: status?.status == FileUploadStatus.error
                    ? Colors.red.withOpacity(0.5) : AppTheme.greenRim.withOpacity(0.5)),
                ),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(_formatBytes(size),
                        style: TextStyle(color: size > 500*1024*1024 ? Colors.red : const Color(0xFF5A8A6A), fontSize: 11)),
                    ])),
                    if (status != null) ...[
                  ...
                  if (status?.status == FileUploadStatus.error)
                    Padding(padding: const EdgeInsets.only(top: 4),
                      child: Text(status!.message, style: const TextStyle(color: Colors.red, fontSize: 11))),
                ]),
              );
            }),
          ],
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            ...
            child: _uploading
              ? const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: AppTheme.gold)),
                  SizedBox(width: 10),
                  Text('Mengupload...', style: TextStyle(color: AppTheme.goldLight)),
                ])
              : const Text('🚀  Upload Semua File', style: TextStyle(color: AppTheme.goldLight, fontSize: 15, fontWeight: FontWeight.w600)),
          )),
  ...
  Widget _sectionTitle(String t) => Row(children: [
    Text(t, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
  ]);
...
class _FileStatus {
  ...
  double progress;
  String message;
  _FileStatus({required this.file, this.status = FileUploadStatus.idle, this.progress = 0, this.message = ''});
}
